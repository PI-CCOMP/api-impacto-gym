...
export async function authMiddleware(
  ...
  if (!header?.startsWith('Bearer ')) {
    res.status(401).json({ erro: 'Token não fornecido.', codigo: 'TOKEN_AUSENTE' })
    return
  }
  ...
  try {
    const { rows } = await pool.query<{ id: number; perfil: Perfil; status: string | null }>(
    ...
    res.status(500).json({ erro: 'Erro interno no servidor.', codigo: 'ERRO_INTERNO' })
  }
}
...
export function authorize(...perfis: Perfil[]) {
  return (req: Request, res: Response, next: NextFunction): void => {
    if (!req.user || !perfis.includes(req.user.perfil)) {
      res.status(403).json({ erro: 'Acesso não autorizado para este perfil.', codigo: 'SEM_PERMISSAO' })
      return
    }
    next()
  }
...
export function requireMfa(req: Request, res: Response, next: NextFunction): void {
  if (!req.user?.mfa_verificado) {
    res.status(403).json({ erro: 'Verificação MFA necessária.', codigo: 'MFA_REQUERIDO' })
    return
  }
  next()
}
